...
uis.InputBegan:Connect(function(input)
    if input.KeyCode == Enum.KeyCode.Space and funcs.fly then
        if lp.Character and lp.Character:FindFirstChild("HumanoidRootPart") then
            lp.Character:FindFirstChild("HumanoidRootPart").Velocity = Vector3.new(0, 50, 0)
        end
    end
end)
...
function CreateHighlight() -- make any new highlights for new players
    for _, v in pairs(players:GetChildren()) do
        if v ~= lp and v.Character and not v.Character:FindFirstChild("Highlight") then
            ...
            highlight.FillColor = Color3.fromRGB(0, 225, 0) -- default green color
        end
    end
end

function UpdateHighlights() -- Update role colors for players
    for _, v in pairs(players:GetChildren()) do
        if v ~= lp and v.Character and v.Character:FindFirstChild("Highlight") then
            local highlight = v.Character:FindFirstChild("Highlight")
            if v.Name == Murder and IsAlive(v) then
                highlight.FillColor = Color3.fromRGB(225, 0, 0) -- red for Murder
